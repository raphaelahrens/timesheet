...
__version__ = (2017, 1, 11, 10, 24, 27, 2)
...
class TimeSheetParser(Parser):
    ...
    def parse(self, text, *args, **kwargs):
        if not isinstance(text, Buffer):
            text = TimeSheetBuffer(text, **kwargs)
        return super(TimeSheetParser, self).parse(text, *args, **kwargs)
    ...
    def _work_line_(self):
        ...
        with self._optional():
            ...
            with self._optional():
                ...
                self.name_last_node('done')
        self.ast._define(
            ['date', 'done', 'end', 'saldo', 'start', 'total', 'weekday'],
            []
        )
...
def main(
    ...
    parser = TimeSheetParser(parseinfo=False)
    ast = parser.parse(
        text,
        startrule,
        filename=filename,
